...
def wrapper_obc_io_create_delete(mcg_obj, awscli_pod, bucket_factory, iterations=1):
    """
    ...
    for i in range(iterations):
        if EXPANSION_COMPLETED:
            ...
            # there will be an assert if things go wrong in the functions called by following function
            obc_io_create_delete(mcg_obj, awscli_pod, bucket_factory)
...
class TestTier1Framework(ManageTest):
    def test_tier1_framework(self, project_factory, multi_dc_pod, multi_pvc_factory, pod_factory,
        ...
        executor_run_bg_ios_ops.submit(test_create_delete_pvcs,multi_pvc_factory, pod_factory,project)

        # Start NooBaa IOs in the background.:
        status_noobaa_s3_io = executor_run_bg_ios_ops.submit(
            wrapper_s3_io_create_delete, mcg_obj, awscli_pod, bucket_factory,200
        )
        logging.info("Started s3_io_create_delete...")

        executor_run_bg_ios_ops.submit(wrapper_obc_io_create_delete, mcg_obj, awscli_pod, bucket_factory,200)
        ...
        if status_noobaa_s3_io.result(timeout=900) and node_status.result(timeout=900):
